...
class Application implements IApplication {
  ...
  private mMainWindow: WeakRef<MainWindowT>;
  ...
  private onShowWindow(): void | undefined {
    return this.showMainWindow(this.mArgs?.startMinimized);
  }
  ...
  private ensureRequiredDirectories(): Result<void> {
    try {
      ...
      this.mStartupLogPath = path.join(temporaryPath, "startup.log");
  ...
  private startUi(): Promise<void> {
    ...
    this.mMainWindow = new WeakRef<MainWindowT>(
      new MainWindow(this.mStore, this.mArgs.inspector)
    );
  ...
  private setupAppEvents(args: IParameters) {
    ...
    app.on("activate", async () => {
      if (this.mMainWindow !== undefined) {
        await this.mMainWindow.deref().create(this.mStore);
      }
    });

    app.on("second-instance", handleAppSecondInstance);

    function handleAppSecondInstance(event: Event, secondaryArgv: string[]) {
    ...
    app.whenReady().then(() => {
      ...
      let userResult = WindowAdminService.tryFindUserData(args, vortexPath);
      let userData: string = "";
      if (userResult.ok) {
  ...
  private regularStart(args: IParameters): Promise<void> {
    ...
    return (
      fs
        ...
        .catch({ code: "ENOSPC" }, handleDiscDriveFull)
        .catch(handleCascadeErrors)
        .finally(() => fs.removeAsync(this.mStartupLogPath).catch(() => null))
    );

    function handleDiscDriveFull() {
      dialog.showErrorBox(
        "Startup failed",
    ...
    function handleCascadeErrors(err: { stack: any; message: any }) {
      try {
  ...
  private connectTrayAndWindow() {
    if (this.mTray.initialized) {
      this.mMainWindow.deref().connectToTray(this.mTray);
    }
  }
      ...
      .then(() => {
        if (backups.length > 0) {
          ...
          return createFullStateBackup("startup", this.mStore)
            ...
            .catch((err) =>
              log("error", "Failed to create startup state backup", err.message)
            );
        }
        return Promise.resolve();
      })
  ...
  showMainWindow(startMinimized: boolean) {
    ...
    try {
      this.mMainWindow.deref().show(maximized, startMinimized);
    } catch (err) {
      if (this.mMainWindow === null) {
    ...
    setWindow(this.mMainWindow.deref().getHandle());
  }
